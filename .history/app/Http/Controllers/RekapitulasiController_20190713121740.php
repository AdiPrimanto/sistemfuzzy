...
class RekapitulasiController extends Controller
{
    public function index()
    {
        ...
        chartGanjil = json_decode(json_encode($data2), True);

        $chartGenap = DB::select('select hasil, count(id_rekapitulasi) from rekapitulasi where semester = "GENAP" GROUP by hasil');
        return json_encode($chartGenap);
