...
class SubmitController extends Controller
{
    public function submit(Request $request){
        ...
        $a = \DB::table('akademik')
                ->where('id_matakuliah', $request->input('matakuliah'))
                ->where('id_kelas', $request->input('kelas'))
                ->where('id_dosen', $request->input('dosen'))
                ->where('id_semester', $request->input('semester'))
                ->first();

        $hasil_kuisioner = new Submit();
        $hasil_kuisioner->id_hasil_kuisioner = $request->input('id_hasil_kuisioner');
        $hasil_kuisioner->id_akademik = $a->id_akademik;
        ...
        $hasil_kuisioner->id_semester = $request->input('id_semester');
